...
public class Deck : MonoBehaviour {
    ...
    [SerializeField]
    List<int> cardIndex;     // The original and untouched list of card indices

    [SerializeField]
    List<int> initialCards;  // initial numbers of each card

    [SerializeField]
    int cardsRemaining;      // num cards remaining

    [SerializeField]
    int numUniqueCards;      // should be length of cardIndex
    ...
    {
        int index = GameController.instance.cardDict.findIndex(name);
        if (index == -1) return -1;
        return draw(index);
    }
